...
enum HttpEnumStatusCode {
  /**
   * The server has received the request headers and the client should proceed to send the
   * request body (in the case of a request for which a body needs to be sent; for example,
   * a POST request). Sending a large request body to a server after a request has been
   ...
   * and is processing the request, but no response is available yet. This prevents the client
   ...
   * for example, a GET request on a form that requires data to be presented via POST, or a
   ...
   * data being encoded as a query-string of a GET request,
   ...
   * RFC 2324, Hyper Text Coffee Pot Control Protocol, and is not expected to be implemented
...
export default HttpEnumStatusCode;
